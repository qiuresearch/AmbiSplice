...
for idx in range(SEQ.shape[0]):
    if sys.argv[2] == '1':
        if NAME[idx].split(b'.')[0] in genes:
            continue
    #exclude orthologs from macaque/mouse training sets
    if NAME[idx].split(b'.')[0] in orthologs and sys.argv[1] != "all":
        continue
    ...
    for i, y in enumerate(A0[0]):
        ...
        # coordinates
        if STRAND[idx] == b'+':
            h5f2.create_dataset('Z' + str(ctr), data=np.array([CHROM[idx], str(int(TX_START[idx])-SL+SL*i-1), str(int(TX_START[idx])-SL+SL*i+SL+CL_max-1), STRAND[idx]], dtype="S"))
        elif STRAND[idx] == b'-':
            h5f2.create_dataset('Z' + str(ctr), data=np.array([CHROM[idx], str(int(TX_END[idx])+SL-SL*i), str(int(TX_END[idx])+SL-SL*i-SL-CL_max), STRAND[idx]], dtype="S"))
        else:
            print(f"error strand: {STRAND[idx]}")
            exit()
