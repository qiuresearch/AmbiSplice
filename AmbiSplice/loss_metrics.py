...
def topks_roc_prc_metrics(y_pred, y_true, ks=(0.5, 1, 2, 4), multiples_of_true=False):
    ...
    argsorted_y_pred = np.argsort(y_pred)  # ascending order
    ...
    for k in ks:
        ...
        if k * idx_true.size < len(y_true):
            tp = np.intersect1d(argsorted_y_pred[-k:], idx_true).size
        else:
            tp = y_true[argsorted_y_pred[-k:]].sum()

        metric['k'] = k
        metric['topk_threshold'] = sorted_y_pred[-k-1:-k+1].mean() if k > 1 else sorted_y_pred[-1]
        metric['topk_accuracy'] = tp / min(k, idx_true.size)
        metric['topk_precision'] = tp / k
        metric['topk_recall'] = tp / idx_true.size
    ...
    for i, idx in enumerate(idx_thresholds):
        ...
        tprs[i] = tp / idx_true.size
        fprs[i] = fp / (len(y_true) - idx_true.size)
...
def calc_metric(preds, labels, keep_batchdim=False, eps=1e-8):
...
def calc_benchmark(preds, labels, keep_batchdim=True, eps=1e-8):
    """ preds are the output of forward() without any activation
        labels are the ground truth in the batch_feats
    """
    benchmark_metrics = calc_metric(preds, labels, keep_batchdim=keep_batchdim, eps=eps)
